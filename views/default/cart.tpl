...
{if ! $rsProducts}
    В корзине пусто.
{else}
    <form action="{$siteUrl}cart/action/order/" method="POST">
        <h2>Данные заказа</h2>
        <table>
            <tr>
                    <th>№</th>
            ...
            {foreach $rsProducts as $item name=products}
                <tr>
                    ...
                    <td><a href="{$siteUrl}product/id/{$item['id']}/">{$item['name']}</a><br /></td>
                    <td>
                        <input name="itemCnt_{$item['id']}" id="itemCnt_{$item['id']}" type="text" value="1" 
                            size="3"  onchange="conversionPrice({$item['id']});"/>
                    </td>
                    <td>
